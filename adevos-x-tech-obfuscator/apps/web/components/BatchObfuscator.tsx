...
export default function BatchObfuscator({ settings, brandName }: BatchObfuscatorProps) {
  ...
  const outputFileName = (f: BatchFile) => {
    const base = f.name.replace(/\.[^.]+$/, "");
    const ext = f.result ? EXTENSION_FOR[f.result.language] : "js";
    return `${base}.obfuscated.${ext}`;
  };

  const handleCopyOne = async (f: BatchFile) => {
    if (!f.result) return;
    await navigator.clipboard.writeText(f.result.code);
    setCopiedId(f.id);
    setTimeout(() => setCopiedId((v) => (v === f.id ? null : v)), 1500);
  };

  const handleDownloadOne = (f: BatchFile) => {
    if (!f.result) return;
    const ext = EXTENSION_FOR[f.result.language];
    const blob = new Blob([f.result.code], { type: MIME_FOR[ext] ?? "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = outputFileName(f);
    a.click();
  ...
  const handleDownloadZip = async () => {
    ...
    const zip = new JSZip();
      ...
      zip.file(outputFileName(f), f.result!.code);
    });
            ...
            {files.map((f) => (
              ...
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  {f.status === "done" && (
                    <>
                      <CheckCircle2 size={16} className="text-neon-500 mr-1" />
                      <button
                        onClick={() => handleCopyOne(f)}
                        aria-label="Copy obfuscated output"
                        title="Copy"
                        className="p-1.5 rounded-md text-slate-400 hover:text-neon-400 hover:bg-charcoal-800 focus-ring"
                      >
                        {copiedId === f.id ? <Check size={14} className="text-neon-400" /> : <Copy size={14} />}
                      </button>
                      <button
                  ...
                  <button
                    ...
                    title="Remove"
                    className="p-1.5 rounded-md text-slate-400 hover:text-red-400 hover:bg-charcoal-800 focus-ring"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
