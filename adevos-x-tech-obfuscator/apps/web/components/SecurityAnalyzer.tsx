...
function QuickActionButton({
  ...
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-lg border border-neon-500/60 text-neon-400 hover:bg-neon-500 hover:text-charcoal-950 hover:border-neon-500 transition-colors focus-ring"
    >
      {children}
      <ArrowRight size={12} />
    </button>
...
export default function SecurityAnalyzer({
  ...
  if (alerts.length === 0) {
    return (
      <div className="flex items-center gap-2 text-sm text-slate-500 border border-charcoal-700 rounded-xl px-3.5 py-2.5">
    ...
    <div className="space-y-2">
      {alerts.map((alert) => {
        ...
        return (
          <div
            key={alert.id}
            className={`border rounded-xl px-3.5 py-3 ${STYLES[alert.type]}`}
          >
